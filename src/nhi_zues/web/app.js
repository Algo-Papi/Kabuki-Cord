      ...
      return `
        <div class="channel-row ${index === selectedChannel ? "active" : ""}">
          ...
          <div class="channel-actions">
            <button class="icon-mini pin ${chan.pinned ? "on" : ""}" data-pin-channel="${index}" title="${chan.pinned ? "Unpin channel" : "Pin channel to top"}" aria-label="${chan.pinned ? "Unpin channel" : "Pin channel to top"}"><i class="bi bi-pin-angle${chan.pinned ? "-fill" : ""}"></i></button>
            <button class="pill-toggle observe ${chan.scan_enabled ? "on" : ""}" data-toggle="scan" data-channel="${index}">Observe</button>
            <button class="pill-toggle react ${chan.react_enabled ? "on" : ""}" data-toggle="react" data-channel="${index}" title="Auto-add capped lightweight reactions to clear jokes, agreement, thanks, or weird claims. Works with Engage off as long as Observe is on; Observe only blocks writes.">React</button>
            <button class="pill-toggle engage ${chan.engage_enabled ? "on" : ""}" data-toggle="engage" data-channel="${index}">Engage</button>
            <button class="pill-toggle auto ${chan.auto_respond_enabled ? "on" : ""}" data-toggle="auto" data-channel="${index}" title="Permit autonomous sends in this channel when the global response mode also allows them.">Auto</button>
          </div>
        </div>
      `;
    })
  ...
  document.querySelectorAll("[data-toggle]").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      if (button.dataset.toggle === "auto") {
        if (!chan.auto_respond_enabled && !confirm("Allow autonomous sends in this channel when the global response mode permits them?")) return;
        chan.auto_respond_enabled = !chan.auto_respond_enabled;
        if (chan.auto_respond_enabled) chan.engage_enabled = true;
      }
      persistServersSoon();
...
function renderOnboardingSteps() {
  ...
  const configuredServers = servers().filter((srv) => {
    const id = String(srv.server_id || "").trim();
    return id && !id.endsWith("_here");
  });
  const configuredChannels = configuredServers.flatMap((srv) => srv.channels || []).filter((chan) => {
    const id = String(chan.channel_id || "").trim();
    return id && !id.endsWith("_here");
  });
  const serversReady = configuredServers.length > 0;
  const engagedChannels = configuredChannels.filter((chan) => chan.scan_enabled || chan.engage_enabled).length;
  const reactionChannels = configuredChannels.filter((chan) => chan.react_enabled).length;
  ...
  const steps = [
    ...
    },
    {
      ...
      ready: serversReady,
      body: serversReady
        ? `${configuredServers.length} server${configuredServers.length === 1 ? " is" : "s are"} configured. Use Repair if a server's channel list is stale.`
    ...
    },
    {
      ...
      body: engagedChannels
        ? `${engagedChannels} channel${engagedChannels === 1 ? " is" : "s are"} enabled. Observe reads; Engage drafts. React is on for ${reactionChannels}.`
    ...
    },
    {
      ...
      body: "Start in Observe only or Review every draft. Use Latest/Backfill to confirm memory before relying on generated drafts.",
    },
    {
      title: "7. Understand privacy and transport limits",
      ready: true,
      body: "Conversation memory and the Discord browser profile stay on this device. AI drafting sends selected conversation context to OpenAI. Personal-account browser automation is inherently less stable and carries more platform risk than a supported integration.",
    },
  ];
    ...
    approvals
      .map((item) => {
        ...
        <div class="approval-item" data-approval-item="${escapeAttr(item.approval_id)}">
          ...
          ${currentRuntimeMode() === "dry" ? `<div class="approval-warning">Observe only is on. Approved messages are blocked until Response Mode changes in API & Runtime.</div>` : ""}
        </div>
      `;
      })
...
function openApprovalReview(approvalId) {
  ...
  renderApprovalReview();
  openDialog("approvalReviewOverlay", "[data-approval-review-draft]");
}

function closeApprovalReview() {
  activeApprovalReviewId = null;
  closeDialog("approvalReviewOverlay");
}
...
async function clearDiscordCredentials() {
  if (!confirm("Remove the stored Discord email and password from the operating system keyring?")) return;
  await api("/api/discord-credentials-clear", { method: "POST", body: JSON.stringify({}) });
  await loadState();
  toast("Stored Discord credentials cleared");
}

async function switchDiscordAccount() {
  const confirmed = confirm(
    "Switch Discord accounts? This signs Kabuki-Cord out by clearing its private browser session and stored Discord credentials. Existing channel automation will be disabled until you sync and re-enable channels. Local history is preserved.",
  );
  if (!confirmed) return;
  const email = $("discordEmail").value.trim();
  const password = $("discordPassword").value;
  await api("/api/discord-session-reset", {
    method: "POST",
    body: JSON.stringify({ confirmation: "SWITCH_DISCORD_ACCOUNT" }),
  });
  if (email || password) {
    await api("/api/discord-credentials", {
      method: "POST",
  ...
  await loadState();
  toast("Fresh Discord sign-in opened. Log into the new account, then Sync Discord.");
}

async function clearOpenAIKey() {
  if (!confirm("Remove the stored OpenAI API key from the operating system keyring?")) return;
  await api("/api/openai-key-clear", { method: "POST", body: JSON.stringify({}) });
  $("apiKey").value = "";
  await loadState();
  toast("Stored OpenAI API key cleared");
}

async function clearLocalState() {
  const typed = prompt('Type CLEAR to delete local conversation and activity state. This cannot be undone.');
  if (typed !== "CLEAR") return;
  const result = await api("/api/local-state-clear", {
    method: "POST",
    body: JSON.stringify({ confirmation: "CLEAR_LOCAL_STATE" }),
  });
  appState = result.state;
  renderStateUpdate();
  toast("Local conversation and activity state cleared");
}
...
function openDialog(id, preferredFocusSelector = "") {
  const overlay = $(id);
  if (!overlay) return;
  dialogReturnFocus = document.activeElement;
  overlay.classList.remove("hidden");
  requestAnimationFrame(() => {
    const preferred = preferredFocusSelector ? overlay.querySelector(preferredFocusSelector) : null;
    (preferred || focusableElements(overlay)[0])?.focus();
  });
}
...
$("switchDiscordAccount").addEventListener("click", () => switchDiscordAccount().catch((error) => toast(error.message)));
...
$("clearOpenAIKey").addEventListener("click", () => clearOpenAIKey().catch((error) => toast(error.message)));
$("clearDiscord").addEventListener("click", () => clearDiscordCredentials().catch((error) => toast(error.message)));
$("clearLocalState").addEventListener("click", () => clearLocalState().catch((error) => toast(error.message)));
...
$("runtimeMode").addEventListener("change", () => {
  ...
  if (["semi_auto", "full_auto"].includes(nextMode)) {
    const accepted = confirm(
      "This mode can send messages without per-message approval in channels where Auto is enabled. Continue?"
    );
    if (!accepted) {
...
$("channelAutoRespond").addEventListener("change", () => {
  const current = channel();
  if (!current) return;
  const enabled = $("channelAutoRespond").checked;
  if (enabled && !confirm("Allow autonomous sends in this channel when the global response mode permits them?")) {
    $("channelAutoRespond").checked = false;
    return;
  }
  current.auto_respond_enabled = enabled;
  if (enabled) current.engage_enabled = true;
  persistServersSoon();
  renderServerPanel();
});
...
$("llmEnabled").addEventListener("change", () => {
  ...
  const accepted = confirm(
    "Enable AI drafting? Selected Discord conversation context will be sent to the configured OpenAI API for generation."
  );
  if (!accepted) $("llmEnabled").checked = false;
});
