...
Per-channel React can run independently of Engage. When Observe and React are enabled for a channel and dry-run is off, Kabuki-Cord can add capped lightweight reactions to clear cues such as jokes, agreement, thanks, or unusually weird claims. The default cap is `NHI_ZUES_REACTION_MAX_PER_CHANNEL=2` per channel scan, and the reaction ledger prevents repeating any app-made reaction on the same Discord message. The **Reaction threshold**, **Random reaction percent**, **Force laugh recent**, and **Reaction emoji override** settings let you make reaction behavior stricter, looser, partially random, force-laugh sample the latest five non-character messages, or fixed to a specific emoji. **Force laugh recent** is a rolling ceiling, not an endless dice roll: if the setting is `40%`, Kabuki will keep reactions at or below 2 of the latest 5 non-character messages until new messages enter that window.
